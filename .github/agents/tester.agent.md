...
whenToUse: |
  - When creating or updating unit tests for `src/` modules or the indexer.
  - When diagnosing and fixing failing tests or flaky tests under `tests/`.
...
examples:
  - "Add unit tests for `src/core/indexing/indexer.ts` covering chunking edge cases."
  - "Fix failing tests in `tests/chunking.test.ts` and make them deterministic."
  - "Create a GitHub Actions workflow that runs `pnpm test` and linters in CI."
selectionHints: |
...
- Add unit tests for `src/core/indexing/indexer.ts` covering chunking edge cases.
- Diagnose and fix the failing tests in `tests/chunking.test.ts`.
- Create a GitHub Actions workflow that runs `pnpm test` and linters in CI.
