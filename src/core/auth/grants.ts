...
export function createGrant(
  ...
  opts: { userId: number; repoId: string; role: GrantRole; branchPattern?: string | null; grantedBy: number },
): Grant {
  ...
  if (existing) {
    ...
    rawDb
      .prepare(
        `INSERT INTO repo_grants (user_id, repo_id, role, branch_pattern, granted_by, created_at)
         VALUES (?, ?, ?, ?, ?, ?)`,
      )
      .run(opts.userId, opts.repoId, opts.role, branchPattern, opts.grantedBy, createdAt)
  }
  const row = rawDb
    .prepare(
      'SELECT id, user_id, repo_id, role, branch_pattern, granted_by, created_at FROM repo_grants WHERE user_id = ? AND repo_id = ? AND branch_pattern IS ?',
    )
...
export function listGrants(rawDb: InstanceType<typeof Database>, repoId: string): Grant[] {
  const rows = rawDb
    .prepare(
      'SELECT id, user_id, repo_id, role, branch_pattern, granted_by, created_at FROM repo_grants WHERE repo_id = ? ORDER BY created_at ASC',
    )
...
export function listGrantsForUser(rawDb: InstanceType<typeof Database>, userId: number): Grant[] {
  const rows = rawDb
    .prepare(
      'SELECT id, user_id, repo_id, role, branch_pattern, granted_by, created_at FROM repo_grants WHERE user_id = ? ORDER BY created_at ASC',
    )
...
export function resolveUserRepoAccess(
  ...
  const grants = rawDb
    .prepare('SELECT id, user_id, repo_id, role, branch_pattern, granted_by, created_at FROM repo_grants WHERE user_id = ? AND repo_id = ?')
