...
export function computeHealthTimeline(dbSession: ReturnType<typeof getActiveSession>, opts: { buckets?: number; branch?: string } = {}): HealthSnapshot[] {
  ...
  const rows = rawDb.prepare('SELECT DISTINCT commit_hash, timestamp FROM commits ORDER BY timestamp ASC').all() as Array<{ commit_hash: string; timestamp: number }>
  ...
  for (let i = 0; i < buckets; i++) {
    ...
    // deadConceptRatio: blobs from this period that are no longer in HEAD (not present in latest commit set)
    const headBlobs = rawDb.prepare('SELECT DISTINCT blob_hash FROM blob_commits WHERE commit_hash = (SELECT commit_hash FROM commits ORDER BY timestamp DESC LIMIT 1)').all() as Array<{ blob_hash: string }>
