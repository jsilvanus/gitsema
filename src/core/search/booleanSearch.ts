/**
 * Boolean/Composite query support — AND, OR over vectorSearch result sets.
 * NOT is handled by --not-like/--lambda (already implemented in vectorSearch).
 */
import type { SearchResult } from '../models/types.js'

/**
 * OR: union of two result sets, keeping max score when a blob appears in both.
 */
...
/**
 * AND: intersection of two result sets, using harmonic mean of scores.
 */
...
/**
 * Parse a simple boolean expression into tokens.
 * Supports: "query1 AND query2", "query1 OR query2"
 * Returns null if no boolean operator is detected (plain query).
 */
