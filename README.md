...
> **Storage backends (experimental, Phase 101):** `storage.backend` selects where the index lives — currently only `sqlite` (the default) is implemented; `postgres` (+ pgvector) and `qdrant` are planned. `storage.scope` chooses which index a command resolves to: `project` (per-repo `.gitsema/`, default), `user` (`~/.gitsema/`), or `named` (an explicitly addressed index via `storage.name`). See [`docs/storage-backends-plan.md`](docs/storage-backends-plan.md).
