...
- `search` (`src/cli/commands/search.ts`, `mapModelLevelToSearchLevel()` + `resolveAgreedLevel()`): consults `getModelProfile(textModel).level`, and — since dual-model search (`textModel !== codeModel`) embeds the query with *both* models and merges results, applying the same `--level` flags to both passes — also `getModelProfile(codeModel).level` when dual-model routing is active. Same conflict rule as the indexing side: agree (or only one set) → use it; genuinely disagree → skip rather than guess. Applied ahead of the `embed_config` auto-recall.
