...
| `--merge-levels` | `search` | bool | false | Merge active search levels into one shared-cutoff ranked list (pre-Phase-136 behavior) instead of separate per-level lists; only meaningful when 2+ of {chunk, symbol, module} are active at once (Phase 136); MCP `semantic_search` (`merge_levels`) / HTTP `POST /search` (`mergeLevels`) since Phase 138 |
