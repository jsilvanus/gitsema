...
blob-first index: typed, queryable, temporally-aware edges between stable symbol
nodes — without violating any of the project's non-negotiable constraints.
...
2. **Blob-first / immutable.** Anything extracted from a blob is computed **once per
   blob hash** and never recomputed (same discipline as embeddings).
...
across file versions, but gitsema is **blob-first / immutable**. We resolve this
with a two-level model that mirrors the existing split between immutable embeddings
(per-blob) and recomputable derived artifacts (clusters, module centroids,
projections).

| Level | What it is | Lifecycle | Identity |
|---|---|---|---|
| **Symbol occurrence** | A symbol as it appears in **one blob version** (today's `symbols` table, enriched) | **Immutable**, dedup'd by blob hash — extracted once per blob, never recomputed | `(blob_hash, start_line, symbol_key)` |
| **Symbol node** | The **logical** symbol tracked across all versions | **Recomputable** — rebuilt by `gitsema graph build` | `symbol_key` |
| **Edge** | A typed relationship between two symbol nodes (or node→external) | **Recomputable**, but carries Git provenance (first/last commit observed) | `(src_key, dst_key, edge_type)` |

### 2.1 `symbol_key` — the stable identity

```
symbol_key = [repo_id "/"]? path "#" qualified_name "#" signature_hash
```

- `repo_id` — present only in multi-repo deployments (omitted in the common
  single-repo case so keys stay readable).
- `path` — the file path the symbol lives at. We deliberately key on **path, not
  blob hash**: the point of a node is to be stable across blob versions of the same
  file. (A file rename mints a new key in v1; rename/move tracking is a later
  refinement — see §8 risks.)
- `qualified_name` — the scope chain joined by `.`:
  `Outer.Inner.method`, `module.func`, `Trait::method`. Requires a **recursive
  AST walk with a scope stack** — the single biggest change to the current
  extractor.
- `signature_hash` — short hash of the **normalized** parameter list (and, where
  cheap, the return type). Disambiguates overloads and reduces accidental
  collisions. Normalization strips whitespace, default values, and parameter names,
  keeping arity + types where the grammar exposes them. For dynamically-typed
  languages (Python/JS) where types are absent, the signature degrades to arity +
  parameter names.

**Why this shape:** it round-trips through all three stores trivially (it is derived
from data the relational store already owns — `paths` + the symbol's own fields),
and it is human-readable in CLI output. It is *not* content-addressed, by design —
that is what makes it stable across edits.
...
entirely from `blob_commits` (two blobs that change in the same commits).
...
### 3.1 Extend `symbols` (occurrences) — migration v23 → v24

Add stable-key inputs to the existing table (all nullable for back-compat; older
rows simply lack them until re-indexed):

```ts
symbols += {
  qualifiedName:   text('qualified_name'),       // "Auth.validateToken"
  signature:       text('signature'),            // normalized param list
  signatureHash:   text('signature_hash'),       // short hash of signature
  parentSymbolKey: text('parent_symbol_key'),    // enclosing scope's symbol_key
  symbolKey:       text('symbol_key'),           // path#qname#sighash (indexed)
}
```

Add an index on `symbol_key` and `(blob_hash, symbol_key)`.
...
structural_refs = sqliteTable('structural_refs', {
  ...
  enclosingSymbolKey:text('enclosing_symbol_key'),  // who made the reference (nullable: top-level)
...
### 3.3 New: `symbol_nodes` + `edges` (recomputable) — migration v25 → v26
...
```ts
symbol_nodes = sqliteTable('symbol_nodes', {
  symbolKey:       text('symbol_key').primaryKey(),
  kind:            text('kind').notNull(),          // function|class|method|... | external
  displayName:     text('display_name').notNull(),
  repoId:          text('repo_id'),
  currentBlobHash: text('current_blob_hash'),       // most-recent occurrence (nullable for external)
  isExternal:      integer('is_external').default(0),// unresolved / 3rd-party target
})
...
- **defines / contains** — recursive walk with a scope stack over
  ...
  stack produces `qualified_name`; the parent frame's key becomes `parent_symbol_key`
  and yields a `contains` edge.
...
| Edge type | Source | Stored | Meaning |
|---|---|---|---|
| `contains` | scope stack | derived | symbol/file owns a nested symbol |
| `defines` | blob → symbol | derived | a file version defines this symbol |
| `imports` | `import` refs | derived | file/symbol imports a module/symbol |
| `calls` | `call` refs | derived | symbol calls another symbol |
| `extends` | heritage | derived | class/struct extends a base |
| `implements` | heritage | derived | class implements an interface/trait |
| `references` | `reference` refs | derived | non-call name reference (best-effort) |
| `co_change` | `blob_commits` | derived | two symbols/blobs change together (weight = co-occurrence count) |
| `similar_to` | embeddings | **optional, materialized later** | semantic neighbor (today computed on the fly; only materialize if traversal needs it) |

`similar_to` is intentionally **not** part of the structural-first scope — it is
already available via `vectorSearch`. It is listed so the planner (Phase 109) can
treat semantic neighbors as edges without duplicating storage.
...
interface GraphStore {
  ...
  subgraph(seed: string, depth: number): Promise<{ nodes: SymbolNode[]; edges: Edge[] }>
}
```
...
| **106** | Per-blob structural extraction | v25 | `structural_refs` populated during `index --graph` for **TS/JS + Python**. Immutable, dedup'd by blob hash. Sites only, no resolution. |
| **107** | Linking pass + `edges` | v26 | `gitsema graph build` resolves refs → typed edges with confidence (§4), materializes `co_change` from `blob_commits`. Queryable `symbol_nodes` + `edges`. |
| **108** | Traversal + CLI/MCP | — | `GraphStore` + recursive CTEs. `gitsema graph callers\|callees\|neighbors\|path <symbol>`. MCP `call_graph` / `graph_neighbors`. Makes `impact` structural. |
| **109** | Cascade query planner | — | `FTS filter → vector expand → graph traversal → merge/rerank`; structural signal added to ranking. |
| **110** | Unified graph UI | — | Render subgraphs in HTML (reuse `htmlRenderer-clusters.ts` force-graph); nodes deep-link into existing per-command HTML views. Folds in the "bind the outputs together" goal. |
...
2. **Stable keys vs. refactors.** A signature change or file rename mints a new
   `symbol_key`, breaking edge continuity. *Mitigation:* acceptable for v1;
   rename/move detection (path+name fallback, git rename data) is a later phase.
